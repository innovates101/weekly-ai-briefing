...
**Major AI vendor announcements are always in scope for Things to Know**, even when they aren't finance-specific on their face — model availability changes, export-control actions, safety/security incidents, and outages/restorations from Anthropic, OpenAI, Google, Meta, Mistral, xAI, etc. Institutions building on these models are directly exposed to vendor availability risk (e.g. a model becoming unavailable in a region overnight due to export controls), so these stories carry the same weight as a fintech product launch. Fetched RSS/API results can under-rank or miss these — for example a same-week Anthropic announcement (redeploying Claude Fable 5 after a June 2026 export-control lift) didn't surface in the top-10 category3 picks even though the pool contained adjacent stories about the same event. When curating Things to Know each week, explicitly check anthropic.com/news, openai.com/news, and other major-lab newsrooms directly rather than relying solely on aggregated search results — don't assume the fetch step already surfaced the week's biggest vendor news.

**Actively look for agentic AI use cases and deployments named by specific banks, insurers, and asset managers** (not just vendors selling to them) — e.g. a named institution's production rollout, a survey with concrete adoption figures, a regulatory filing. Generic "AI in banking" think-pieces from consultancies are usually not fresh news and should be treated with the same 7-day-freshness skepticism as any other story (check the underlying event date, not just the page's crawl/publish date — many gets re-syndicated weeks or months after the original event).
